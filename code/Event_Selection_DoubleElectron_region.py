...
for filename in filenames:
    try:
        ...
        if "Events" in f:
            ...
            # Triggers of Electrons
            HLT_Ele27_WPTight_Gsf.append(events["HLT_Ele27_WPTight_Gsf"].array())
...
HLT_Ele27_WPTight_Gsf = ak.concatenate(HLT_Ele27_WPTight_Gsf, axis=0)  
...
def process_file(dataset='default', IS_DATA=True):
    ...
    events_passed_num_electron = 0
    events_passed_electron = 0
    events_passed_m_inv = 0
    ...
    for i in range(len(recoil_magnitude_u)):
        ...
        # Cuts: Single Electron Region
        if num_electrons[i] == 2:  
            events_passed_num_electron += 1
        else:
            continue 
            
        if ak.all(electron_pt[i] > 30) and ak.all(abs(electron_eta[i]) < 2.4):
            
            pt1, pt2 = electron_pt[i]
            eta1, eta2 = electron_eta[i]
            phi1, phi2 = electron_phi[i]
            
            delta_phi = abs(bound_phi(phi1 - phi2))
            delta_eta = eta1 - eta2

            mass2 = 2 * pt1 * pt2 * (np.cosh(delta_eta) - np.cos(delta_phi))
            m_inv = np.sqrt(mass2) 
            
            events_passed_electron += 1
        else:
            continue 

        # Applying the cut on the invariant mass
        if 71 < m_inv < 111:
            events_passed_m_inv += 1
        else:
            continue
        ...
        selected_invariant_mass.append(m_inv)
    ...
    print(f"Events after number of electrons cut: {events_passed_num_electron}")
    print(f"Events after double electron cut: {events_passed_electron}")
    print(f"Events after invariant mass cut: {events_passed_m_inv}")
...
def save_file(dataset_name='default', IS_DATA=True):
    ...
    #Invariant Mass
    mydict['invariant_mass'] = selected_invariant_mass
