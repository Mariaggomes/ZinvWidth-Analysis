...
for filename in filenames:
    try:
        ...
        if "Events" in f:
            ...
            # Triggers of Muons
            HLT_IsoMu24.append(events["HLT_IsoMu24"].array())
            HLT_IsoTkMu24.append(events["HLT_IsoTkMu24"].array())
...
HLT_IsoMu24 = ak.concatenate(HLT_IsoMu24, axis=0)  
HLT_IsoTkMu24 = ak.concatenate(HLT_IsoTkMu24, axis=0)  
...
num_muons = ak.num(muon_pt, axis=1) 
...
def process_file(dataset='default', IS_DATA=True):
    ...
    events_passed_num_muon = 0
    events_passed_muon = 0
    ...
    for i in range(len(recoil_magnitude_u)):
        ...
        # Cuts: Double Muon Region
        if num_muons[i] == 2:  
            events_passed_num_muon += 1 
        else:
            continue 
            
        if ak.all(muon_pt[i] > 30) and ak.all(abs(muon_eta[i]) < 2.4):
            
            pt1, pt2 = muon_pt[i]
            eta1, eta2 = muon_eta[i]
            phi1, phi2 = muon_phi[i]
    ...
    print(f"Events after number of muons cut: {events_passed_num_muon}")
    print(f"Events after double muon cut: {events_passed_muon}")
