...
def process_file(dataset='default', IS_DATA=True):
    ...
    for i in range(len(recoil_magnitude_u)):
        ...
        # Cuts: Single Muon Region
        if num_muons[i] == 1: 
            muon_selected = (muon_pt[i] > 25) & (abs(muon_eta[i]) < 2.4)
            events_passed_muon += 1
        else:
            continue  
            
        # Transverse Mass
        delta_phi_e_met = abs(bound_phi(muon_phi[i] - met_phi[i]))  # Phase difference between the muon and MET
        mt = np.sqrt(2 * muon_pt[i] * met_pt[i] * (1 - np.cos(delta_phi_e_met)))  
    ...
    print(f"Events after single muon cut: {events_passed_muon}")
