...
def process_file(dataset='default', IS_DATA=True):
    ...
    for i in range(len(recoil_magnitude_u)):
        ...
        if num_electrons[i] == 1:  
            electron_selected = (electron_pt[i] > 30) & (abs(electron_eta[i]) < 2.4)
            events_passed_electron += 1
        else:
            continue  
            
        if met_pt[i] > 100:
            events_passed_met_pt += 1  
        else:
            continue
            
        # Transverse Mass
        delta_phi_e_met = abs(bound_phi(electron_phi[i] - met_phi[i]))  # Phase difference between the electron and MET
        mt = np.sqrt(2 * electron_pt[i] * met_pt[i] * (1 - np.cos(delta_phi_e_met)))  
    ...
    print(f"Events after single electron cut: {events_passed_electron}")
    print(f"Events after Met cut: {events_passed_met_pt}")
