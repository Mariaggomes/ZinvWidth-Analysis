...
input_files = [file for file in glob.glob("*.txt") if file not in ["Cert_278820-284044_13TeV_Legacy2016_Collisions16GH_JSON.txt", "pileup_latest.txt"]]

# Iterar sobre cada arquivo TXT
for input_file in input_files:
    # Extrair o nome base (sem extensão) para nomear o CSV de saída
    dataset_name = input_file.replace(".txt", "")
    
    print(f"Processando o arquivo: {input_file}")
    
    # Ler o arquivo TXT e obter os nomes dos arquivos ROOT
...
# Aplicar a máscara a todos os arrays
...
def salvar_file(dataset_name='default', IS_DATA=False):
    """
    Processa e salva os dados em CSV, lidando com estruturas irregulares para exportação correta.
    """
    ...
    # Lista completa com todos os eventos irregulares
    irregular_keys = [
        # Muons
        ...
        # MET e recoil (para segurança em estrutura irregular)
        'recoil_magnitude_u',
    ...
    # Adiciona os campos irregulares convertidos para listas padrão
    for key in irregular_keys:
        try:
            array_masked = eval(f"{key}[mask_4jets]")
            mydict[key] = ak.to_list(array_masked)
        except Exception as e:
            print(f"[Aviso] Variável '{key}' não pôde ser convertida: {e}")
    ...
    print(f"Dados salvos: {dataset_name}.csv")

# Call the function by passing the dataset name and whether it is given or Monte Carlo
salvar_file(dataset_name=dataset_name, IS_DATA=False)
